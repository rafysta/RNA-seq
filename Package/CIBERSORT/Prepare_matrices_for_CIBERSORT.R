#!/usr/bin/Rscript
# Prepare Quantile normalized matrices for CIBERSORT
...
if(FLAG_ensemble){
  ...
  index <- rownames(mat) %in% (D_gene %>% pull(EnsembleID))
  mat <- mat[index,]
  rownames(mat) <- D_gene %>% slice(match(rownames(mat), EnsembleID)) %>% pull(gene_symbol)
}
